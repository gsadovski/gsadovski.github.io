...
[^trace-reversed]: $ \bar{h}^{(1)} \equiv h^{(1)} - \frac{1}{2} \eta \tr \left(h^{(1)}\right) $ is the *trace-reversed* representation of $h^{(1)}$. The name is justified because $\tr(\bar{h}^{(1)}) = -\tr\left(h^{(1)}\right)$. This is exactly the same relationship the Einstein tensor and the Ricci tensor have in four spacetime dimensions. Namely, $\mathrm{Ein} = \mathrm{Ric}- \frac{1}{2}g \tr \left( \mathrm{Ric} \right)$.

### The linear plane wave

The most straightforward solution to \eqref{eq:linear-g-wave} is the so-called *monochromatic*[^monochromatic] *linear plane wave*

$$ h^{(1)} = \mathrm{Re} \left( A e^{i k \cdot \mathbb{x}}\right) \;,  $$ 

where $A$ is a symmetric rank-2 tensor field known as the *polarization tensor*, $k$ is the wave vector --- we've mentioned it before! ---, and $\mathbb{x}$ is the Euler vector field.

Monochromatic means $h^{(1)}$ has a well-defined constant frequency. Consequentially, for a plane wave, $k$ is a globally constant field and a trivial solution to $\nabla k = 0$. Moreover, \eqref{eq:linear-g-wave} is equivalent to $g \left(k,k\right) h^{(1)} = 0$. For $h^{(1)}$ to be a non-trivial solution, $g(k,k)$ must vanish. This reveals that linear plane g-waves do follow null trajectories (represent massless modes), and satisfy the sufficient conditions for $\left(X, \eta + h^{(1)}\right)$ to be classified as a pp-wave spacetime.


In a plane wave, $k \cdot \left( \mathbb{x} - \mathbb{x}_0 \right) = 0$ where $\mathbb{x}_0$ is a constant field. This is a plane equation that defines a family of wave fronts $ \left\\{k \cdot \mathbb{x}_0 = \text{const} \right\\} $.

which happens to be globally constant for a monochromatic g-wave travelling through empty $\left(X,\eta\right)$. Moreover, $k$ is the wave vector --- we've mentioned it before!. The wave vector is always normal to the wave front, so it always points to the direction of propagation. For a monochromatic g-wave, $k$ also happens to be globally constant, which definitely solves the necessary pp-wave condition $\nabla k = 0$. Finally, $\mathbb{x}(x) \; ; \; dx^{\mu}\left(\mathbb{x}\right) \equiv x^{\mu} $ is the Euler vector field.

[^monochromatic]: Monochromatic (single color) means a wave with a single well-defined constant frequency.
### Polarization modes
