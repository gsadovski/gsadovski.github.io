...
It should be clear by now that real g-wave phenomena is extremely complex. In fact, when serious astrophysicists want to realistically simulate their production, they have no choice but to numerically solve the full set of Einstein's field equations in a supercomputer. Unfortunately, I am not a serious astrophysicist. The g-waves we're going to discuss next can be written down in terms of elementary functions like $\sin(x)$, $\cos(x)$, $e^x$, *etc*. So, mathematically, they are very nice. However, now that we know that "simple" mathematics just cannot cope with the complexities real g-waves exhibit, we also know that these nice solutions do not really exist in nature. Hopefully, you can still find them illuminating like I do.

## PP-wave spacetimes

Let us begin by dispelling the myth that $R_{\mu \nu} = 0$ needs to be linearized, in order to get mathematically nice g-wave solutions. There is a whole class of exact g-wave solutions of Einstein's field equations, known as *plane-fronted waves withs parallel propagation* (pp-waves).
